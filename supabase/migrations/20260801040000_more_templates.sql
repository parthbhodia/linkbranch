-- Four more starting templates: Press Sheet, Golden Hour, Broadsheet, Tide Pool.
--
-- profiles_template_check enumerates the allowed ids by name, so a new template
-- is rejected at the database until it is listed here. The themes themselves
-- need no schema: they compose from the existing font, button-shape and
-- background options already carried in theme_config.
...
-- claim_draft carries its own copy of the allowed ids, and silently falls back
-- to the existing template when it does not recognise one -- so a claimed draft
-- would quietly lose a new template rather than fail. Recreated verbatim from
-- 20260730120000 with only the list widened.
  ...
  set display_name = coalesce(
      ...
      template = case
        when draft.payload->>'template'
          in (
            'field-notes', 'after-dark', 'soft-studio', 'signal-deck',
            'press-sheet', 'golden-hour', 'broadsheet', 'tide-pool'
          )
